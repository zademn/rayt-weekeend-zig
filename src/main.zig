...
pub fn cover_img() !void {
    ...
    _ = random;
    const ground_material = Lambertian{ .albedo = Vec3F{ .data = .{ 0.5, 0.5, 0.5 } } };
    _ = ground_material;
    var ai: isize = -11;
    while (ai < 11) : (ai += 1) {
        ...
        while (bi < 11) : (bi += 1) {
            ...
            const choose_mat = prng.random().float(F);
            const center = Vec3F{ .data = .{ a + 0.9 * prng.random().float(F), 0.2, b + 0.9 * prng.random().float(F) } };
            if ((center.sub(Vec3F{ .data = .{ 4, 0.2, 0 } })).norm() > 0.9) {
                if (choose_mat < 0.8) {
                    // diffuse
                    const albedo = Vec3F.random().mul(Vec3F.random());
                    std.debug.print("albedo: {}\n", .{albedo});
                    var sphere_material = Material{ .lambertian = Lambertian{ .albedo = albedo } };
                    ...
                    const fuzz = prng.random().float(F) / 2;
                    var sphere_material = Material{ .metal = Metal{ .albedo = albedo, .fuzz = fuzz } };
                    try world.append(Hittable{ .sphere = Sphere{ .center = center, .material = &sphere_material, .radius = 0.2 } });
                } else {
                    // glass
                    var sphere_material = Material{ .dielectric = Dielectric{ .refraction_index = 1.5 } };
                    try world.append(Hittable{ .sphere = Sphere{ .center = center, .material = &sphere_material, .radius = 0.2 } });
                }
            }
        }
    ...
    var material1 = Material{ .dielectric = Dielectric{ .refraction_index = 1.5 } };
    try world.append(Hittable{ .sphere = Sphere{ .center = Vec3F{ .data = .{ 0, 1, 0 } }, .material = &material1, .radius = 1.0 } });
    var material2 = Material{ .lambertian = Lambertian{ .albedo = Vec3F{ .data = .{ 0.4, 0.2, 0.1 } } } };
    try world.append(Hittable{ .sphere = Sphere{ .center = Vec3F{ .data = .{ -4, 1, 0 } }, .material = &material2, .radius = 1.0 } });
    var material3 = Material{ .metal = Metal{ .albedo = Vec3F{ .data = .{ 0.7, 0.6, 0.5 } }, .fuzz = 0.0 } };
    ...
    const cam = Camera.new(aspect_ratio, image_width, samples_per_pixel, max_depth, vfov, lookfrom, lookat, vup, focus_dist, defocus_angle, &prng);
...
pub fn exercices() !void {
    ...
    const alloc = gpa.allocator();
    // World
    var material_ground = Material{ .lambertian = Lambertian{ .albedo = Vec3F{ .data = .{ 0.8, 0.8, 0.0 } } } };
    // var material_left = Material{ .metal = Metal{ .albedo = Vec3F{ .data = .{ 0.8, 0.8, 0.8 } }, .fuzz = 0.3} };
    // var material_center = Material{ .lambertian = Lambertian{ .albedo = Vec3F{ .data = .{ 0.7, 0.7, 0.3 } } } };
    var material_right = Material{ .metal = Metal{ .albedo = Vec3F{ .data = .{ 0.8, 0.6, 0.2 } }, .fuzz = 0.0 } };
    var material_left = Material{ .dielectric = Dielectric{ .refraction_index = 1.5 } };
    // var material_center = Material{ .dielectric = Dielectric{ .refraction_index = 1.5 } };
    var material_center = Material{ .lambertian = Lambertian{ .albedo = Vec3F{ .data = .{ 0.1, 0.2, 0.5 } } } };
